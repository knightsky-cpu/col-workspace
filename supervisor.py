...
unsupported, or prohibited. Explicit memory intent creates a candidate; policy
decides whether it is approvable. Durable profile candidates must be grounded
in explicit, reusable user requests about the user, their collaboration with
Agent Col, their goals, preferences, interests, standing instructions,
relevant working context, or allowed light identity details. Pure
external-topic, assistant-opinion, informational questions,
...
those clauses. Use existing
structured categories
when they fit; otherwise use user_requested_memory for safe explicit
user-requested memory. Failure to match a predefined category is not by itself
unsupported. Do not infer memory from behavior, history, projects, tool output,
retrieved content, or model-authored text. Treat temporary instructions as
session_only. Workspace requirements are workspace_note. Treat sensitive data
as prohibited. Unsupported is for explicit memory requests that are neither
durable profile memory, session-only instruction, workspace note, nor
prohibited. Chat may not delete or revoke active durable memory; direct the
user to the Memory UI for confirmed revoke/delete actions.
