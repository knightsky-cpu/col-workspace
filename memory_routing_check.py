...
async def run_routing_check(
    *,
    ...
    """Run stateless scenarios and classify typed routing evidence."""
    ...
    if not selected or any(
        scenario.execution_mode != "stateless" for scenario in selected
    ):
        output("memory-routing-check configuration_error")
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--scenario",
        help="Run one stateless scenario by ID.",
    )
    parser.add_argument(
        "--repetitions",
        type=int,
...
def main(
    ...
    *,
    ...
    """Run the live M7.5A baseline with content-free reporting."""
