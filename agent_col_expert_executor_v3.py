"""Parallel routing-v3 expert execution boundary."""
...
class AgentColExpertExecutorV3ConfigurationError(RuntimeError):
    """Raised when configured experts disagree with routing availability."""


def build_requirements_verification_input(
    directive: AgentColRoutingDirective,
    routing_input: AgentColRoutingInput,
) -> RequirementsVerificationInput:
    """Resolve selected text blocks and assign request-scoped identities."""
    validate_routing_directive_for_input(directive, routing_input)
    intent = directive.requirements_verification_intent
    if intent is None:
        raise AgentColExpertExecutorV3ConfigurationError(
            "Requirements Verification intent is required."
        )
    ...
    requirements = tuple(
        RequirementInput(
            ...
            text=candidates_by_id[block_id].text,
...
def build_computation_expert_input(
    ...
    """Resolve validated v3 numeric candidate IDs into one expert request."""
    validate_routing_directive_for_input(directive, routing_input)
...
class AgentColExpertExecutorV3:
    """Execute zero or one expert selected by a validated v3 directive."""
    ...
    def __init__(
        ...
        *,
        ...
        requirements_verification_service: (
            RequirementsVerificationService | None
        ) = None,
    ) -> None:
        self._source_service = source_service
        self._research_service = research_service
        self._computation_service = computation_service
        self._requirements_verification_service = (
            requirements_verification_service
        )
    ...
    def available_capabilities(self) -> tuple[ExpertCapability, ...]:
        ...
        if self._requirements_verification_service is not None:
            capabilities.append(ExpertCapability.REQUIREMENTS_VERIFICATION)
    ...
    async def execute(
        ...
        return await self._execute_requirements_verification(
            directive,
            routing_input,
        )
    ...
    async def _execute_requirements_verification(
        self,
        directive: AgentColRoutingDirective,
        routing_input: AgentColRoutingInput,
    ) -> AgentColResponderContextV3:
        service = self._requirements_verification_service
        if service is None:
            raise AgentColExpertExecutorV3ConfigurationError(
                "Selected Requirements Verification capability is not "
                "configured."
            )
        try:
            request = build_requirements_verification_input(
                directive,
                routing_input,
            )
        except ValidationError:
