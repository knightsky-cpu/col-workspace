...
This document presents the authoritative, source-faithful architectural reconnaissance report for migrating **Agent Col**'s intelligence and inference layer from Google (Gemini, Vertex AI, Google ADK, Google Search grounding, Google STT/TTS) to **OpenAI** (Responses API, GPT-5.6 model family, Strict Structured Outputs, Function Calling, Web Search, Audio APIs).

---

## A. Executive Recommendation

The recommended architecture for Agent Col is a **Local-First, Application-Owned Provider Substitution** using the **OpenAI Responses API** (`AsyncOpenAI` SDK) placed behind a narrow `OpenAIProviderAdapter`.

```
                    ┌────────────────────────────────────────────────────────┐
                    │               AGENT COL APPLICATION                    │
                    │                                                        │
                    │  ┌──────────────────┐      ┌────────────────────────┐  │
...
2. **Stateless Provider Model:** Requests to OpenAI are treated as inference over server-assembled context. OpenAI-hosted session state is explicitly bypassed with `store=false` to guarantee local data ownership.
...
The table below catalogs every Google/Gemini/Vertex/ADK dependency currently present across the repository:

| Source File Path | Google Dependency | Application Contract Depending On It | Classification | Current Responsibility & Behavior |
| :--- | :--- | :--- | :--- | :--- |
| `vertex_config.py` | `google.genai.Client`, Vertex AI SDK | `VertexAISettings` | `authentication`, `deployment-only` | Validates `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, `GOOGLE_GENAI_USE_ENTERPRISE`. Instantiates shared `genai.Client`. |
...
| `agent_col_artifact_feedback_executor.py` | `google.genai.Client`, `genai.types` | `AgentColArtifactFeedbackExecutor` | `provider adapter`, `structured output` | Background worker queue handler processing feedback-driven artifact revisions. |
...
requirements_verification.py    GenAI SDK Requirements Validation           Responses API + gpt-5.6-terra + Schema
...
### GPT-5.6 Reasoning Continuation Benchmark Evaluation

OpenAI GPT-5.6 reasoning models generate private internal reasoning tokens. To determine the optimal multi-turn reasoning configuration when `store=false` / ZDR is active, Agent Col will benchmark two candidate strategies:

#### Strategy A (Preferred Baseline Candidate): `reasoning.context="current_turn"`
- **Mechanism:** Reasoning is evaluated on a per-turn basis (`reasoning.context="current_turn"`). Agent Col's local `working_state` snapshot, continuity service, and transcript history provide context without replaying provider-side reasoning tokens.
...
#### Strategy B (Evaluation Benchmark Candidate): `reasoning.context="all_turns"`
- **Mechanism:** Uses `store=false` while locally persisting and replaying opaque encrypted reasoning output items provided by the Responses API across subsequent requests.
...
OpenAI's **Strict Structured Outputs** (`strict: true`) guarantees 100% schema compliance against Pydantic models. In the Responses API, structured outputs are configured via the `text.format` block:
...
The workload matrix evaluates candidate assignments across the current **GPT-5.6** model family (`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`). Final model selections must be validated through benchmark evaluations prior to production rollout.
...
| **v4 Intent Routing** | `gpt-5.6-luna` | `none` / `low` | Ultra-fast classification, minimal latency, 100% strict JSON schema compliance. | < 350ms | No |
...
Agent Col currently uses Google Search grounding inside `ResearchExpertService`. This will be replaced using OpenAI's built-in **Web Search Tool** (`web_search_preview`):
...
OpenAI Web Search annotations contain `url`, `title`, and optional `snippet` fields. The `OpenAIProviderAdapter` maps these into Agent Col's existing `CitationReceipt` without assuming `snippet` is always present:
...
CitationReceipt(
    ...
    snippet=getattr(annotation, "snippet", None) or "",
)
```
...
| :--- | :--- | :--- | :--- |
| **Responses API (Inference)** | Zero Data Retention eligible (with ZDR account setting) | **Fully Compatible** | Set `store=false` on all requests. |
| **Automatic Prompt Caching** | Transient in-memory cache (billed per write/read) | **Fully Compatible** | Track `cached_tokens` & `cache_write_tokens`. |
| **Audio APIs (`gpt-transcribe` / `gpt-4o-mini-tts`)** | Governed by API ZDR / MAM account policy | **Fully Compatible** | Verifiable under ZDR account policy. |
| **Conversations API** | Hosted session retention | **Incompatible** | **Do Not Use.** Keep history in PostgreSQL. |
| **OpenAI Vector Stores / File Search** | Hosted vector storage | **Incompatible** | **Do Not Use.** Keep search indices local. |
| **Agents SDK Cloud Tracing** | Cloud trace retention | **Incompatible** | **Do Not Use.** Use local application logging. |
...
| `requirements_verification.py` | Contract Validation | Update to OpenAI Structured Output | Provider-only | Low |
...
| `requirements.txt` | Runtime Dependencies | Replace `google-genai`, `google-adk` with `openai` | Deployment-only | Low |
...
2. **Schema Equivalence Validation:** Verify that Pydantic models validated from OpenAI `strict: true` output produce 100% identical dictionary structures as previous GenAI outputs.
...
2. **Multi-Turn Reasoning Benchmark:** Execute Strategy A (`current_turn`) versus Strategy B (`all_turns`) benchmark evaluations to verify multi-turn context quality.
...
4. **Web Search Citation Completeness:** Verify the frequency of missing `snippet` fields in Web Search annotations to refine fallback UI formatting.

---

## S. Sol Verification Corrections
...
| :--- | :--- | :--- | :--- | :--- |
| **1. Model Family** | Hardcoded legacy GPT-4o / o3-mini matrix. | Evaluates current **GPT-5.6** model family (`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`) with candidate reasoning effort levels. | OpenAI Models API Documentation | Implementation Detail |
...
| **4. Reasoning Context** | Fixed `reasoning.context="current_turn"` as a strict requirement. | Made `reasoning.context="current_turn"` (Strategy A) the preferred baseline requiring benchmark comparison against Strategy B (`all_turns`). | OpenAI Reasoning Models Guide | Implementation Detail |
...
| **11. Web Search Citations** | Assumed `snippet` is always present in annotations. | Corrected mapping to handle optional `snippet` fields gracefully in `CitationReceipt`. | OpenAI Web Search Tool Reference | Implementation Detail |
