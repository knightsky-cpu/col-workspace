"""Parallel routing-v2 responder context with computation projection."""
...
ResponderExpertResultV2 = Annotated[
    SourceExpertResult | ResearchExpertResult | ComputationResponderResult,
...
class AgentColResponderContextV2(BaseModel):
    """Validated routing-v2 context allowed into responder-only Agent_Col."""
    ...
    expert_result: ResponderExpertResultV2 | None = None
    ...
    def require_route_specific_context(self) -> Self:
        ...
        if route in {AgentColRoute.DIRECT, AgentColRoute.CLARIFY}:
        ...
        if (
            ...
            raise ValueError("Expert result does not match the selected route.")
...
def build_agent_col_responder_v2_model_context(
    context: AgentColResponderContextV2,
) -> types.Content:
    """Render one bounded server-validated routing-v2 context part."""
    payload = json.dumps(
